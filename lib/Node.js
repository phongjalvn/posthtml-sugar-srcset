...
class Node {
  ...
  getAttribute(name) {
    return this.node.attrs[name] || this.node.attrs[`data-${name}`];
  }

  setAttribute(name, value) {
    if (this.node.attrs[name]) this.node.attrs[name] = value;
    else this.node.attrs[`data-${name}`] = value;
  }

  removeAttribute(name) {
    delete this.node.attrs[name];
    delete this.node.attrs[`data-${name}`];
  }

  hasSrcSet() {
    const srcset =
      this.getAttribute("srcset") || this.getAttribute("data-srcset");
  ...
  fixSrcset() {
    ...
    if (this.hasSrcSet()) {
      if (resolution.srcset || responsive.srcset) this.fixSrcsetFromSrcset();
  ...
  resolutionCallback(ratio) {
    ...
    ratio === 1 && resolution.skip1xSuffix ? url.suffix() : url.suffix(suffix);

    return ratio === 1 ? url.format() : url.format() + " " + ratio + "x";
  }
